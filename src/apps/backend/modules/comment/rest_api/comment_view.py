...
class CommentView(MethodView):
    ...
    def post(self, account_id: str, task_id: str) -> ResponseReturnValue:
        ...
        # TODO: Add content length validation if needed..

        create_comment_params = CreateCommentParams(account_id=account_id, task_id=task_id, content=content)
    ...
    def get(self, account_id: str, task_id: str, comment_id: Optional[str] = None) -> ResponseReturnValue:
        if comment_id:
            ...
            if page and page < 1:
                raise CommentBadRequestError("Invalid page number")

            if size and size < 1:
                raise CommentBadRequestError("Invalid page size")
    ...
    def patch(self, account_id: str, task_id: str, comment_id: str) -> ResponseReturnValue:
        ...
        update_comment_params = UpdateCommentParams(
            account_id=account_id, task_id=task_id, comment_id=comment_id, content=content.strip()
        )
