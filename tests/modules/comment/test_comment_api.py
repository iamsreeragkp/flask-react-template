...
class TestCommentApi(BaseTestComment):
    ...
    def test_get_all_comments_with_comments(self) -> None:
        ...
        assert items[0]["content"] == "Comment 3"
        assert items[1]["content"] == "Comment 2"
        assert items[2]["content"] == "Comment 1"
    ...
    def test_get_all_comments_with_pagination(self) -> None:
        ...
        # Make sure we got different comments
    ...
    def test_update_comment_success(self) -> None:
        ...
        assert response.status_code == 200
        self.assert_comment_response(
            response.json, id=created_comment.id, task_id=task.id, account_id=account.id, content="Updated Content"
        )
    ...
    def test_comments_are_account_isolated_via_api(self) -> None:
        ...
        # Verify comment is still accessible by the owner
    ...
    def test_comments_are_task_isolated(self) -> None:
        ...
        comment1 = self.create_test_comment(account_id=account.id, task_id=task1.id, content="Task 1 Comment")
        comment2 = self.create_test_comment(account_id=account.id, task_id=task2.id, content="Task 2 Comment")

        # Check task1 comments
        ...
        assert response1.json["items"][0]["content"] == "Task 1 Comment"

        # Check task2 comments
        ...
        assert response2.json["items"][0]["content"] == "Task 2 Comment"

        # This should fail - wrong task
        response3 = self.make_authenticated_request("GET", account.id, task2.id, token, comment_id=comment1.id)
        self.assert_error_response(response3, 404, CommentErrorCode.NOT_FOUND)
